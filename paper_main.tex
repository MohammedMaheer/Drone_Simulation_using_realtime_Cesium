...
Every figure in this paper comes out of one archived \texttt{.mat}
log through \texttt{generate\_paper\_figures.m}; the random seeds,
config struct, and mission profile travel with that log. The
framework is a single MATLAB process, which keeps a few things out
of scope for now: aerodynamic coupling between closely spaced
rotors (or between vehicles in a swarm), flexible-airframe modes
that start to matter above $\sim 10$\,kg, and sensor-fusion stacks
beyond the 12-state EKF. On a stock 8-core laptop the standard
60\,s mission runs at roughly $0.6\times$ real time; turning off
the dashboard and the Cesium bridge drops that below $0.25\times$,
which is what we use for batched Monte-Carlo sweeps.
