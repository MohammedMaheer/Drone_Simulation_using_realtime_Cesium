...
Simulation is kept separate from rendering --- they communicate only
through telemetry logs and a JSON state file. Four visualisers are
provided: a pre-flight launcher, a live dashboard, an animated 3-D
replay, and a CesiumJS globe.

\subsection{Pre-flight launcher}
A dark configuration window lets the user pick a preset, edit
mass/arm/prop/Kv/capacity, choose a real-world location, pick a
flight mode (manual or auto-stabilised), and toggle wind, traffic,
online map tiles, and the precise battery model before launching
(Fig.~\ref{fig:launcher}).

\begin{figure}[!htb]
\centering
\includegraphics[width=0.95\columnwidth]{fig14_launcher_ui.png}
\caption{Pre-flight setup UI: preset (or custom airframe),
real-world location, flight mode, and environment options before
pressing FLY.}
\label{fig:launcher}
\end{figure}

\subsection{Live telemetry dashboard}
Fig.~\ref{fig:dash} captures the live MATLAB dashboard at takeoff
($t=5$\,s), cruise ($t=25$\,s), and completion ($t=59$\,s). It
refreshes at $50$\,Hz via \texttt{animatedline} and shows attitude,
...
\texttt{animate\_flight.m} reuses the parametric 3-D model from the
live simulator. Fig.~\ref{fig:anim} is a mid-mission frame with a
telemetry overlay alongside the 3-D scene; Fig.~\ref{fig:replay}(b)
is the full waypoint mission as a 3-D ground/air track, and
Fig.~\ref{fig:replay}(c) is the standard six-panel
\texttt{plot\_flight\_data} summary (ground track, altitude, speed,
attitude, motor speeds, battery curve) emitted at the end of every
run.
...
MATLAB state to a CesiumJS WebGL canvas~\cite{cesium}. The drone is
rendered on Google 3-D Tiles at the chosen latitude/longitude/altitude,
with the same attitude quaternion driving the model.
Fig.~\ref{fig:cesium} shows it running over Bengaluru.

\begin{figure}[!htb]
\centering
\includegraphics[width=0.98\columnwidth]{fig17_cesium_globe.png}
