...
\begin{figure*}[!htb]
\centering
\includegraphics[width=0.82\textwidth]{fig18_pipeline_overview.png}
...
\begin{figure*}[!htb]
\centering
\includegraphics[width=0.86\textwidth]{fig15_drone_3d_models.png}
...
\begin{figure*}[!htb]
\centering
\includegraphics[width=0.52\textwidth]{fig14_launcher_ui.png}
...
\begin{figure*}[!htb]
\centering
\includegraphics[width=0.62\textwidth]{fig17_cesium_globe.png}
...
summarises the six-panel diagnostic. Position trace is a bounded
random walk dominated by GPS innovation noise; body angular rates
remain below 5\,$^\circ$/s, and the four-motor RPM band stays within
$\pm$3\,\% spread. Battery SOC falls almost linearly and total power
tracks the analytic hover estimate to within 1.5\,\%. Repeating with
\texttt{mini\_quad} and \texttt{heavy\_hex} presets yields the same
qualitative shape scaled by mass and disc loading, confirming the
auto-tuner produces consistent gains across the airframe envelope.

\begin{figure*}[!htb]
\centering
\includegraphics[width=0.82\textwidth]{fig1_hover_stability.png}
...
2.4\,m on the upwind legs. Altitude regulation degrades more
gracefully: the barometer-aided EKF holds the cruise height within
$\pm$0.4\,m even at the worst-case turbulence intensity. The Dryden
filter contributes a broadband component centred near 3\,Hz visible
as attitude ripple, while the deterministic gust overlay produces
the characteristic low-frequency body-axis sway. Mission completion
time grows by roughly 4\,\% and no motor saturates the configured
thrust ceiling. Re-running on \texttt{mini\_quad} and
\texttt{heavy\_hex} airframes yields 1.6$\times$ and 0.7$\times$ the
radial deviation respectively, illustrating how the arbitrary-$N$
mixer transparently scales the disturbance-rejection budget across
the airframe envelope.
...
hover power matches the analytical estimate (216\,W at 1.5\,kg) within
1.5\,\%. The model resolves $V_{oc}(\mathrm{SOC})$ as a piecewise
polynomial, the ohmic drop $R_0 i$, and a single RC pair
$(R_1, C_1)$ for the relaxation transient. Coulomb counting is
corrected by a slow voltage-feedback term to bound long-horizon
drift. Winding temperature feeds back into $R_m(T_m)$ inside the BLDC
model so that sustained high-throttle commands appear as a
measurable reduction in mechanical efficiency.
...
is tracked with $<$1$^\circ$ RMS. The four motor traces split cleanly
into pairs during yaw transients, confirming correct distribution of
the yaw moment. Peak commanded slew stays below
$\dot\omega_{\max}=2500$\,rad/s$^2$, bounding peak phase current. In
still air the residual tracking error is essentially Gaussian and
consistent with the IMU noise floor mapped through the estimator gain.

\subsection{Reproducibility and limitations}
All figures are regenerated by \texttt{generate\_paper\_figures.m}
from a single archived \texttt{.mat} log; random seeds, configuration
struct, and mission profile are persisted alongside the log. The
framework is single-process MATLAB and does not yet cover (i)
aerodynamic interaction between closely spaced rotors or swarms,
(ii) flexible-airframe modes for lifters above $\sim$10\,kg, and
(iii) sensor-fusion architectures beyond the 12-state EKF. End-to-
end runtime is approximately 0.6$\times$ wall clock on a stock
8-core laptop; disabling the dashboard and Cesium bridge brings
real-time-factor below 0.25$\times$ for batched Monte-Carlo sweeps.
